...
class Emarket_Menu {
	...
	function emarket_menu_upload(){
		wp_enqueue_script(
			'emarket-menu-upload-js',get_template_directory_uri().'/lib/admin/js/field_upload.js',
						...
						?><p class="field-<?php echo esc_attr( $field ); ?> description description-wide">
								...
								switch( $field_opts['type'] ):
									...
									?>			
											...
											echo ' <a href="javascript:void(0);" class="emarket-menu-upload button-secondary"'.$upload.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.esc_html__('Browse', 'emarket').'</a>';
											echo ' <a href="javascript:void(0);" class="emarket-menu-upload-remove"'.$remove.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.esc_html__('Remove', 'emarket').'</a>';
											?>								
									<?php
										break;	
										case 'checkbox':
...
class Emarket_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ( preg_match('/(apc|apcs)/i', $advanced) ){
			...
			$page_content	 = empty( ( class_exists( 'JP_User_Menus' ) || class_exists( 'Nav_Menu_Roles' ) ) || $page_query ) ? '' : $page_query->post_content;
